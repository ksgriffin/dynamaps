...
function build_input(opts){
        ...
        for( var i=opts.min; i<= opts.max; i++){
         ...
         opts.images[i].onload = function(i){this.loaded = true; console.log("loaded"+this.varno+" "+this.imgno); check_loaded_button(useroptions[opts.div_id],this.varno,this.imgno );};
         opts.images[i].src = opts.prefix+opts.timecode+opts.values[i]+opts.extension
          }
